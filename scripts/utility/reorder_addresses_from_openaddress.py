...
with open(sample_csv, newline='') as csvfile:
    ...
    for row in tqdm.tqdm(csv_reader):
        ...
        if add1_val:
            add1_val = add1_val.replace("FIRST", "1st").replace("SECOND", "2nd").replace("THIRD", "3rd").replace("FOURTH", "4th").replace("FIFTH", "5th")
...
with open(csv_path, newline='') as csvfile:
    csv_reader = csv.reader(csvfile, delimiter=';')
    _ = next(csv_reader)
    header = ["name", "domain", "industry", "country", "city", "address1", "address2", "postcode"]
