package com.ssdgen.generator.documents.data.helper;
...
public class HelperCommon extends Helper {
    ...
    public static String getResourceFullPath(Object classObj, String resourcePath) throws Exception {
        ...
        // so it must be converted to URI that gives a working path with %20 convereted
        // to ' '
     ...
     * that states whether the component (key) should be used in the layout
     * generation
     * 
     ...
     */
    public static Map<String, Boolean> getMatchedConfigMap(List<Map<String, Object>> configMaps, String layout)
            throws Exception {
    ...
    /*
     * When rendering non ANSI text, the useANSIEncoding must be set to false when
     * using SimpleTextBox
     */
    public static PDCustomFonts getNonANSIRandomPDFontFamily(PDDocument doc, Object classObj) throws Exception {
        ...
        for (List<String> fontNBI : ttfFontNBIList) {
            pdfontNormalList.add(PDType0Font.load(doc,
                    new File(HelperCommon.getResourceFullPath(classObj, "common/font/" + fontNBI.get(0)))));
            pdfontBoldList.add(PDType0Font.load(doc,
                    new File(HelperCommon.getResourceFullPath(classObj, "common/font/" + fontNBI.get(1)))));
            pdfontItalicList.add(PDType0Font.load(doc,
                    new File(HelperCommon.getResourceFullPath(classObj, "common/font/" + fontNBI.get(2)))));
        }
