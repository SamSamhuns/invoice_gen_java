...
public class ClientInfoBox extends ElementBox {
    ...
    public ClientInfoBox(PDFont font, PDFont fontB, float fontSize, InvoiceModel model, PDDocument document,
    ...
    private void init() throws Exception {
        ...
        if (addressType.equals("Billing") && noBillHeading > 6) {
            ...
            heading = new SimpleTextBox(fontB, fontSize + 2, 0, 0,
                    model.callviaName(client_obj, "get" + addressType + "Head").toString());

        container.addElement(heading);

        // container.addElement(new BorderBox(Color.WHITE,Color.WHITE, 0,0, 0, 0, 5));

        SimpleTextBox name = new SimpleTextBox(fontB, fontSize, 0, 0,
                model.callviaName(client_obj, "get" + addressType + "Name").toString());
        name.setEntityName(entityPrefix + "N");
        container.addElement(name);

        SimpleTextBox adresse1 = new SimpleTextBox(font, fontSize, 0, 0, model
                .callviaName(model.callviaName(client_obj, "get" + addressType + "Address"), "getLine1").toString());
        adresse1.setEntityName(entityPrefix + "A");
        container.addElement(adresse1);

        addLine2 = model.callviaName(model.callviaName(client_obj, "get" + addressType + "Address"), "getLine2")
                .toString();
        ...
        addLine3 = model.callviaName(model.callviaName(client_obj, "get" + addressType + "Address"), "getLine3")
                .toString();
        ...
        SimpleTextBox zip = new SimpleTextBox(font, fontSize, 0, 0,
                model.callviaName(model.callviaName(client_obj, "get" + addressType + "Address"), "getZip").toString());
        zip.setPadding(0, 0, 5, 0);
        zip.setEntityName(entityPrefix + "A");
        cityContainer.addElement(zip);

        SimpleTextBox city = new SimpleTextBox(font, fontSize, 0, 0, model
                .callviaName(model.callviaName(client_obj, "get" + addressType + "Address"), "getCity").toString());
        city.setEntityName(entityPrefix + "A");
        cityContainer.addElement(city);
        container.addElement(cityContainer);

        acountry = model.callviaName(model.callviaName(client_obj, "get" + addressType + "Address"), "getCountry")
                .toString();
