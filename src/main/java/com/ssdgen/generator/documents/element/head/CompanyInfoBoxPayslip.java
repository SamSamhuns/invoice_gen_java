...
public class CompanyInfoBoxPayslip extends ElementBox {
    ...
    public CompanyInfoBoxPayslip(HorizontalContainer hcontainer) {
        this.hcontainer = hcontainer;
    }

    public CompanyInfoBoxPayslip(VerticalContainer container) {
        this.container = container;
    }

    public CompanyInfoBoxPayslip(PDFont font, PDFont fontB, float fontSize, Model model, PDDocument document)
            throws Exception {
        this.font = font;
        this.fontB = fontB;
        ...
        // note getResource returns URL with %20 for spaces etc, so it must be converted
        // to URI that gives a working path with %20 convereted to ' '
        URI uri = new URI(this.getClass().getClassLoader()
                .getResource("common/logo/" + model.getCompany().getLogo().getFullPath()).getFile());
    ...
    public VerticalContainer getCompanyAddressBlock() throws Exception {
        ...
        if (model.getCompany().getAddress().getLine2() != null
                && model.getCompany().getAddress().getLine2().length() > 0) {
        ...
        if (model.getCompany().getAddress().getLine3() != null
                && model.getCompany().getAddress().getLine3().length() > 0) {
        ...
        if (model.getCompany().getAddress().getCountry() != null
                && model.getCompany().getAddress().getCountry().length() > 0) {
            SimpleTextBox country = new SimpleTextBox(font, fontSize, 0, 0,
                    model.getCompany().getAddress().getCountry(), "SA");
    ...
    public HorizontalContainer getCompanyAddLine() throws Exception {
        ...
        if (model.getCompany().getAddress().getLine2() != null
            ...
            SimpleTextBox adresse2 = new SimpleTextBox(font, fontSize, 0, 0,
                    separator + model.getCompany().getAddress().getLine2(), "SA");
            // adresse2.setEntityName("SA");
        ...
        if (model.getCompany().getAddress().getLine3() != null
            ...
            SimpleTextBox adresse3 = new SimpleTextBox(font, fontSize, 0, 0,
                    separator + model.getCompany().getAddress().getLine3(), "SA");
            // adresse3.setEntityName("SA");
            adresse3.setPadding(0, 0, 2, 0);
            address.addElement(adresse3);
        }

        SimpleTextBox zip = new SimpleTextBox(font, fontSize, 0, 0,
                separator + model.getCompany().getAddress().getZip(), "SA");
        ...
        if (model.getCompany().getAddress().getCountry() != null
                && model.getCompany().getAddress().getCountry().length() > 0) {
            SimpleTextBox country = new SimpleTextBox(font, fontSize, 0, 0,
                    separator + model.getCompany().getAddress().getCountry(), "SA");
            // country.setEntityName("SA");
    ...
    public VerticalContainer getCompanyContactBlock() throws Exception {
        ...
        if (model.getCompany().getContact().getPhoneValue() != null
                && model.getCompany().getContact().getPhoneValue().length() > 0) {
            HorizontalContainer phoneContainer = new HorizontalContainer(0, 0);
            SimpleTextBox phoneLabel = new SimpleTextBox(font, fontSize, 0, 0,
                    model.getCompany().getContact().getPhoneLabel());
            phoneLabel.setPadding(0, 0, 5, 0);
            phoneContainer.addElement(phoneLabel);
            SimpleTextBox phoneValue = new SimpleTextBox(font, fontSize, 0, 0,
                    model.getCompany().getContact().getPhoneValue(), "SCN");
        ...
        if (model.getCompany().getContact().getFaxValue() != null
                && model.getCompany().getContact().getFaxValue().length() > 0) {
            HorizontalContainer faxContainer = new HorizontalContainer(0, 0);
            SimpleTextBox faxLabel = new SimpleTextBox(font, fontSize, 0, 0,
                    model.getCompany().getContact().getFaxLabel());
            faxLabel.setPadding(0, 0, 5, 0);
            faxContainer.addElement(faxLabel);
            SimpleTextBox faxValue = new SimpleTextBox(font, fontSize, 0, 0,
                    model.getCompany().getContact().getFaxValue(), "SFAX");
    ...
    public VerticalContainer getCompanyIdBlock() throws Exception {
        ...
        for (String idName : idNames) {
            ...
            if (idName.equals("Rcs")) // Special because not available in IDNumbers
            {
                ...
                SimpleTextBox RCSValue1 = new SimpleTextBox(font, fontSize, 0, 0,
                        model.getCompany().getAddress().getCity(), "SA");
                ...
                SimpleTextBox Label = new SimpleTextBox(font, fontSize, 0, 0,
                        model.callviaName(idnumObj, labelName).toString());
                ...
                SimpleTextBox Value = new SimpleTextBox(font, fontSize, 0, 0,
                        model.callviaName(idnumObj, valueName).toString(), "S" + idName.toUpperCase());
    ...
    public HorizontalContainer getCompanyIdLine(int size) throws Exception {
        ...
        for (int i = 0; i < idNames.length; i++) {
            if (idNames[i].equals("Rcs")) // Special because not available in IDNumbers
            {
                ...
                companyIDContainer.addElement(Label);
                SimpleTextBox Value = new SimpleTextBox(font, fontSize, 0, 0,
                        model.callviaName(idnumObj, valueName).toString(), "S" + idNames[i].toUpperCase());
